...
mutation_plot<-function(start, stop, text="", drop=-0.15, col="red") {
   rect(start, -0.08, stop, drop, col="grey", border="grey")
...
for ($i = 1; $i <= $gene_n; $i++) {
  ...
  foreach $k (sort {$a<=>$b} keys %BP ){
    ...
    print OUTR "mutation_plot($k, $k, \">$k\", $d, col=\"black\")\n";
    $old = $k;
  }
}
print OUTR "dev.off()\n";
