...
open OUTR,">LDheatmap-$ARGV[3]-$Phenotype.R" or die $!;
print OUTR <<EOF;
if (! require("LDheatmap")) install.packages("LDheatmap")
if (! require("genetics")) install.packages("genetics")
data <- read.table("tmp-$ARGV[3]-$Phenotype.txt", header=F,sep="\t",row.names=1)
NAs <- data == "N/N"
is.na(data)[NAs] <- TRUE
data[NAs] <- NA
...
pdf(file="Plot-LDheatmap-$ARGV[3].pdf", width=6, height=4)
...
dev.off()
EOF
system("Rscript LDheatmap-$ARGV[3]-$Phenotype.R");
print "Plotted Gene LDheatmap to file Plot-LDheatmap-$ARGV[3].pd\n\n";
